...
struct LibraryView: View {
        ...
        NavigationStack {
            ...
            .toolbar {
                ToolbarItem(placement: .navigationBarTrailing) {
                    PhotosPicker(selection: $pickerItems, maxSelectionCount: 200, matching: .images) {
                        Image(systemName: "square.and.arrow.down").a11yLabel("Import with Photo Picker")
                    }
                }
                ToolbarItem(placement: .navigationBarTrailing) {
                    Button(action: { vm.toggleSort() }) {
                        Image(systemName: vm.sortByDate ? "calendar" : "text.magnifyingglass").a11yLabel("Toggle sort")
                    }
                }
            }
            .settingsLink()
    ...
    func handlePicker(items: [PhotosPickerItem]) async {
        ...
        await vm.importImages(uiImages)
    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
            if let image = UIImage(contentsOfFile: asset.filePath) {
                ...
                Rectangle().fill(Color(.secondarySystemBackground)).frame(height: 110)
            }
