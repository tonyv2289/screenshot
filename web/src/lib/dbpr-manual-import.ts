...
type ManualImportFileType = "condo" | "payment";

export type DbprManualImportFile = {
  fileName: string;
  body: string;
};

export type RunDbprManualImportResult = {
  runId: string;
...
export type RunDbprManualImportOptions = {
  jobName?: string;
};
...
function detectFileType(fileName: string): ManualImportFileType | null {
  const normalized = normalizeFileName(fileName);
  if (!normalized.endsWith(".csv")) return null;
  if (normalized.startsWith("condo_")) return "condo";
  if (normalized.startsWith("paymenthist_")) return "payment";
  return null;
}

export function isSupportedDbprImportFileName(fileName: string): boolean {
...
async function ingestCondoFile(client: PoolClient, fileName: string, body: string) {
...
async function ingestPaymentFile(client: PoolClient, fileName: string, body: string) {
  const parsed = parse(body, {
    columns: true,
    skip_empty_lines: true,
    relax_column_count: true,
...
export async function runDbprManualImport(
  files: DbprManualImportFile[],
  options: RunDbprManualImportOptions = {},
): Promise<RunDbprManualImportResult> {
  const pool = getDbPool();
  if (!pool) {
    throw new Error("DATABASE_URL is required for manual DBPR import.");
  }

  const client = await pool.connect();
  const jobName = options.jobName || "ingest_dbpr_manual_upload";
  ...
  try {
    for (const file of files) {
      ...
      if (fileType === "condo") {
        totalProcessed += await ingestCondoFile(client, file.fileName, file.body);
      } else {
        totalProcessed += await ingestPaymentFile(client, file.fileName, file.body);
      }
      filesProcessed += 1;
    }

    if (filesProcessed === 0) {
      throw new Error("No supported DBPR files were provided. Use Condo_*.csv or paymenthist_*.csv.");
    }
      ...
      `
        ...
        SET
          ...
          details = jsonb_build_object(
            'files_processed', $3::integer,
            'files_skipped', $4::integer,
            'uploaded_files', $5::jsonb
          )
        WHERE id = $1
      `,
      [runId, totalProcessed, filesProcessed, filesSkipped, JSON.stringify(files.map((item) => item.fileName))],
    );
    ...
    await client.query(
      ...
      [
        ...
        JSON.stringify(files.map((item) => item.fileName)),
      ],
    );
